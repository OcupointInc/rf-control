...
`rf-control` provides two normal RF operations: CW and continuous sweep. In
both modes the software fixes the LO at 9600 MHz and safely mutes the output
...
The frequency is in MHz and must be 9500–11500 MHz.
...
Start and stop are in MHz. Sweep time must include a unit, such as `10s`,
`20ms`, or `35us`.

## Output level

At 0 dB attenuation the calibrated nominal output is −25 dBm. Each command
restores that calibrated synthesizer-power baseline automatically. Add
`--attenuation` to either command to reduce it in 0.25 dB steps:

```bash
rf-control --ip 192.168.1.253 cw \
  --frequency 10000 --attenuation 6.25
```
...
```bash
rf-control --ip 192.168.1.253 cw \
  --frequency 10000 --clock external
```
...
Status shows the selected clock, LO, CW/sweep mode, attenuation, nominal output
estimate, lock state, and controller temperature.
...
- For external-clock errors, connect and enable the 10 MHz reference or retry
  with `--clock internal`.
- The firmware normally powers the LO down at startup. This is expected; every
  `cw` or `sweep` command wakes it and sets it to 9600 MHz automatically.
