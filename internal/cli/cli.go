...
const (
	lmx2595RegisterCount = 113
	lmx2595RegisterBatch = 16
)

// parseLMXRegisterArgs accepts the same compact notation shown in the GUI:
// individual addresses (0 or R0), comma-separated lists, and inclusive ranges.
// No addresses means the complete R0..R112 register map.
func parseLMXRegisterArgs(args []string) ([]uint32, error) {
	if len(args) == 0 {
		all := make([]uint32, lmx2595RegisterCount)
	...
	parseAddress := func(value string) (uint32, error) {
		value = strings.TrimSpace(value)
		value = strings.TrimPrefix(strings.TrimPrefix(value, "R"), "r")
		parsed, err := strconv.ParseUint(value, 0, 32)
		if err != nil || parsed >= lmx2595RegisterCount {
			return 0, usagef("invalid LMX2595 register %q (expected R0..R112)", value)
		}
	...
	for _, arg := range args {
		for _, item := range strings.Split(arg, ",") {
			...
			if item == "" {
				return nil, usagef("empty LMX2595 register address")
			}
			parts := strings.Split(item, "-")
			if len(parts) > 2 {
				return nil, usagef("invalid LMX2595 register range %q", item)
			}
			...
			if len(parts) == 2 {
				...
				if last < first {
					return nil, usagef("invalid descending LMX2595 register range %q", item)
				}
			}
	...
	addresses := make([]uint32, 0, len(seen))
...
type lmxRegisterValue struct {
	Address uint32 `json:"address"`
	Value   uint32 `json:"value"`
	Hex     string `json:"hex"`
}

func cmdReadLMX(args []string) error {
	fs := flag.NewFlagSet("read-lmx", flag.ExitOnError)
	common := &commonFlags{}
	addCommonFlags(fs, common)
	jsonOutput := fs.Bool("json", false, "Print machine-readable JSON")
	_ = fs.Parse(args)
	addresses, err := parseLMXRegisterArgs(fs.Args())
	if err != nil {
		return err
	}
	...
	registers := make([]lmxRegisterValue, 0, len(addresses))
	for start := 0; start < len(addresses); start += lmx2595RegisterBatch {
		end := start + lmx2595RegisterBatch
		if end > len(addresses) {
			end = len(addresses)
		}
		response, err := c.ReadLMX2595Registers(addresses[start:end])
		if err != nil {
			return fmt.Errorf("read LMX2595 registers R%d..R%d: %w", addresses[start], addresses[end-1], err)
		}
		if len(response.GetAddresses()) != len(response.GetValues()) || len(response.GetAddresses()) != end-start {
			return fmt.Errorf("LMX2595 register response contained %d addresses and %d values for a %d-register request", len(response.GetAddresses()), len(response.GetValues()), end-start)
		}
		for i, address := range response.GetAddresses() {
			if address != addresses[start+i] {
				return fmt.Errorf("device returned LMX2595 register R%d where R%d was requested", address, addresses[start+i])
			}
			value := response.GetValues()[i]
			registers = append(registers, lmxRegisterValue{Address: address, Value: value, Hex: fmt.Sprintf("0x%04X", value)})
		}
	}
	if *jsonOutput {
		encoded, err := json.MarshalIndent(struct {
			Registers []lmxRegisterValue `json:"registers"`
		}{Registers: registers}, "", "  ")
		if err != nil {
			return err
		}
...
// cmdMockWhalepod runs the stateful protobuf simulator used for GUI development
// and end-to-end testing. It binds to loopback by default and is intentionally
// listed only in engineering help.
func cmdMockWhalepod(args []string) error {
	fs := flag.NewFlagSet("mock-whalepod", flag.ExitOnError)
	listen := fs.String("listen", "127.0.0.1:5000", "Loopback TCP address for the mock control server")
	_ = fs.Parse(args)
	if fs.NArg() != 0 {
		return usagef("usage: mock-whalepod [--listen 127.0.0.1:5000]")
	}
	host, _, err := net.SplitHostPort(*listen)
	if err != nil || (host != "127.0.0.1" && host != "localhost") {
		return usagef("mock-whalepod must listen on loopback as host:port")
	}
	server, err := mockfirmware.ListenWhalepod(*listen)
	if err != nil {
		return err
	}
	defer server.Close()
	fmt.Printf("Whalepod mock firmware listening at %s\n", server.Addr())
...
func cmdMockAirshark(args []string) error {
	fs := flag.NewFlagSet("mock-airshark", flag.ExitOnError)
	listen := fs.String("listen", "127.0.0.1:5000", "Loopback TCP address for the mock control server")
	_ = fs.Parse(args)
	if fs.NArg() != 0 {
		return usagef("usage: mock-airshark [--listen 127.0.0.1:5000]")
	}
	host, _, err := net.SplitHostPort(*listen)
	if err != nil || (host != "127.0.0.1" && host != "localhost") {
		return usagef("mock-airshark must listen on loopback as host:port")
	}
	server, err := mockfirmware.ListenAirshark(*listen)
	if err != nil {
		return err
	}
	defer server.Close()
	fmt.Printf("Airshark mock firmware listening at %s\n", server.Addr())
	fmt.Println("Connect the GUI directly to 127.0.0.1. Press Ctrl+C to stop.")
	return server.Serve()
}
...
func applyBatch(c *client.Client, cfg *batchConfig) (*applyResult, error) {
	...
	if cfg.Barracuda != nil {
		...
		if cfg.Barracuda.RFEnabled != nil {
			if !*cfg.Barracuda.RFEnabled {
				if err := c.SetLoFrequency(0); err != nil {
					return fail("barracuda.rf_enabled", err)
				}
			} else {
				if err := c.SetLoFrequency(client.BarracudaFixedLOMHz); err != nil {
					return fail("barracuda.rf_enabled", err)
				}
				if err := c.SetLMXOutputPower(client.BarracudaCalibratedLMXPowerCode); err != nil {
					_ = c.SetLoFrequency(0)
					return fail("barracuda.rf_enabled", err)
				}
			}
			res.Applied = append(res.Applied, fmt.Sprintf("barracuda.rf_enabled=%v", *cfg.Barracuda.RFEnabled))
		}
	}
...
Commands:
  ...
  read-lmx [REGISTERS]   Read live LMX2595 registers without changing hardware.
                         Accepts addresses/ranges such as 0-15,44,R75; defaults
                         to all R0..R112. Add --json for machine-readable output.
...
// Main runs the command-line interface using os.Args and exits with the
// documented command status when an operation fails.
func Main() {
