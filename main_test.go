...
func TestFormatGPIOSelfTestGrouped(t *testing.T) {
	// A full whalepod_automation pin set (8 pins).
	auto := func(vhfPassed bool) *pb.GpioSelfTestResponse {
		return &pb.GpioSelfTestResponse{
			AllPassed: vhfPassed,
			Pins: []*pb.GpioPinResult{
				{Pin: 0, Name: "PWR_EN", Passed: true},
				{Pin: 2, Name: "SCK", Passed: true},
				{Pin: 3, Name: "MOSI", Passed: true},
				{Pin: 7, Name: "CS_VHF", Passed: vhfPassed, Stuck: pb.GpioStuckState_GPIO_STUCK_STATE_LOW},
				{Pin: 8, Name: "CS_UHF", Passed: true},
				{Pin: 12, Name: "CAL_SW", Passed: true},
				{Pin: 15, Name: "CAL_AMP_EN", Passed: true},
				{Pin: 9, Name: "CLOCK_EN", Passed: true},
			},
		}
	}

	// All pass: one PASS line per subsystem, no per-pin spell-out.
	got := formatGPIOSelfTestGrouped("whalepod_automation", auto(true))
	...
	got = formatGPIOSelfTestGrouped("whalepod_automation", auto(false))
	for _, want := range []string{
